...
public static void findPreSuc(Node root, int key)
    ...
    public static Node p(Node root, int  key , Node min){
        if(root == null) return min;
        if(root.data>=key) return p(root.left , key , min);
        else return p(root.right , key , root);
    }
    
    public static Node s(Node root, int key , Node max){
        if(root == null) return max;
        if(root.data>key) return s(root.left , key , root); 
        else return s(root.right , key , max);
    }
}
